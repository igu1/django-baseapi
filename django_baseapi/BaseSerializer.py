...
class BaseSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, model: Type[Model] = None, fields: list[str] | None = None, depth: int = None, **kwargs): 
        ...
        if self._get_foreign_keys(model) and not depth:
    ...
    def _calculate_depth(self, model: Type[Model], visited: set = None) -> int:
        ...
        for field in model._meta.get_fields():
            if isinstance(field, ForeignKey):
                ...
                max_depth = max(max_depth, self._calculate_depth(related_model, visited)) 
        return max_depth + 1
