...
class BaseAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.login_user and request.user.is_authenticated:
            ...
            return Response(
                {
                    "message": "If you want to use this feature, please login or remove login_user parameter"
                },
                status=401,
            )
        ...
        objects = (
            ExtremeFilter(self.model).filter(request)
            if self.extreme_filter
            else objects
        )
        serializer = BaseSerializer(
            objects, many=True, model=self.model, fields=self.fields, depth=self.depth
        )
        return Response(serializer.data)
